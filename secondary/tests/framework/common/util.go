...
func ValidateClusterState(asm AlternateShardMap) map[InvalidClusterState][]string {
	...
	for defnId, defnStruct := range asm {
		...
		if len(defnStruct.ReplicaMap) != defnStruct.NumReplica+1 {
			err := fmt.Sprintf("less replicas %v than definition %v for index: %v", len(defnStruct.ReplicaMap), defnStruct.NumReplica+1, idx)
			log.Printf("ValidateClusterState: %v", err)
			res[MISSING_REPLICA_INVALID_CLUSTER_STATE] = append(res[MISSING_REPLICA_INVALID_CLUSTER_STATE], err)
		}

		for replicaId, partnMap := range defnStruct.ReplicaMap {
			// could be a case of lost parititions
			if len(partnMap) != defnStruct.NumPartition {
				err := fmt.Sprintf("less partitions %v than definition %v for index: %v", len(partnMap), defnStruct.NumPartition, idx)
				log.Printf("ValidateClusterState: %v", err)
				res[MISSING_PARTITION_INVALID_CLUSTER_STATE] = append(res[MISSING_PARTITION_INVALID_CLUSTER_STATE], err)
			}

			for _, asis := range partnMap {
				if defnStruct.IsPrimary && len(asis) != 1 {
					err := fmt.Sprintf("Alternate Shard ID count mismatch for primary index. Expected 1 actual %v for index: %v", asis, idx)
					log.Printf("ValidateClusterState: %v", err)
					res[ALTERNATE_SHARD_AFFINITY_INVALID_CLUSTER_STATE] = append(res[ALTERNATE_SHARD_AFFINITY_INVALID_CLUSTER_STATE], err)
				} else if !defnStruct.IsPrimary && len(asis) != 2 {
					err := fmt.Sprintf("Alternate Shard ID count mismatch for index. Expected 2 actual %v for index: %v", asis, idx)
					log.Printf("ValidateClusterState: %v", err)
					res[ALTERNATE_SHARD_AFFINITY_INVALID_CLUSTER_STATE] = append(res[ALTERNATE_SHARD_AFFINITY_INVALID_CLUSTER_STATE], err)
				}
				...
				for _, asiStr := range asis {
					...
					if asi.ReplicaId != uint8(replicaId) {
						err := fmt.Sprintf("replica ID mismatch for Alternate Shard ID. Expected %v Actual %v for index: %v", replicaId, asi.ReplicaId, idx)
						log.Printf("ValidateClusterState: %v", err)
						res[ALTERNATE_SHARD_AFFINITY_INVALID_CLUSTER_STATE] = append(res[ALTERNATE_SHARD_AFFINITY_INVALID_CLUSTER_STATE], err)
					}
				}
