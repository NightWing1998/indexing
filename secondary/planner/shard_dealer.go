...
func (sd *ShardDealer) GetSlot(defnID c.IndexDefnId,
	...
	// // if more than half of nodes are above soft limit, then we should create a new slot
	// if len(nodesUnderSoftLimit)*2 < len(nodesForShard) {
	// 	logging.Tracef("ShardDealer::GetSlot many nodes above soft limit %v. creating new slot for defnID %v",
	// 		nodesUnderSoftLimit, defnID)
