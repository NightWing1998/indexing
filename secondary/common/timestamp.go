...
func init() {
	tsSz = int64(unsafe.Sizeof(TsVbuuid{}))
}

func (ts *TsVbuuid) Size() int64 {
	size := tsSz
	numVbs := int64(ts.NumVBuckets)
	size += int64(len(ts.Bucket)) + int64(len(ts.CollectionId))
	size += numVbs * 64     // len(vbuuid) * size of uint64
	size += numVbs * 64     // len(seqnos) * size of uint64
	size += numVbs * 64 * 2 // len(snapshots) * size of [2]uint64
	size += int64(len(ts.OSOCount) * 64)

	// TODO: cal for ManifestUID -> len(string) + 24 for each string in ManifestUID array. 24 for string struct storage
