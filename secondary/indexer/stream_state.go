...
type StreamState struct {
	...
	//last time when stream merge check happened
	streamKeyspaceIdLastMergeCheckTime map[common.StreamId]KeyspaceIdLastMergeCheckTime
...
func (ss *StreamState) initNewStream(streamId common.StreamId) {
	...
	keyspaceIdLastMergeCheckTime := make(KeyspaceIdLastMergeCheckTime)
	ss.streamKeyspaceIdLastMergeCheckTime[streamId] = keyspaceIdLastMergeCheckTime
...
func (ss *StreamState) initKeyspaceIdInStream(streamId common.StreamId,
	keyspaceId string, numVBuckets int) {
...
func (ss *StreamState) cleanupKeyspaceIdFromStream(streamId common.StreamId,
	...
	delete(ss.streamKeyspaceIdLastMergeCheckTime[streamId], keyspaceId)
...
func (ss *StreamState) computeRollbackTs(streamId common.StreamId, keyspaceId string) *common.TsVbuuid {
	...
	if rollTs, ok := ss.streamKeyspaceIdKVRollbackTsMap[streamId][keyspaceId]; ok && rollTs.Len() > 0 {
		...
		if restartTs == nil {
			numVBuckets := ss.streamKeyspaceIdNumVBuckets[streamId][keyspaceId]
